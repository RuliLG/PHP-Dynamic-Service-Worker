...
function h($file, &$version = null) {
	...
	if (@$timestamp) {
		...
		// Add -timestamp -timestamp at the end of the filename
		$parts = pathinfo($file);
		$folder = str_replace($parts['basename'], '', $file);
		$path = $folder.$parts['filename'].'-hsh'.$timestamp.'.'.$parts['extension'];
		return $path;
	}
	return $file;
}
...
function parse_text($txt) {
	...
	$txt = strtolower(str_replace(array_keys($transliterationTable), array_values($transliterationTable), $txt));
